...
// ----------------------------------------
import Step1Component from './BookingComponents/Step1Component';
import Step2Component from './BookingComponents/Step2Component';
import Step3Component from './BookingComponents/Step3Component';
...
function BookingPage (){
    ...
    const handleReset = async () => {
      ...
      // {patientDetails.map(val => console.log(val))}
      try{
        const response = await axios.post(`${process.env.REACT_APP_API_URL}/api/patient/createpatient/`,patientDetails)
        {alert(`Appointment has been confirmed`)}
        {navigate('/')}
        setActiveStep(0);
      }catch(error){
        alert(`Check if all the fields are`)
      }
    };
    ...
    function getSteps(index){
      switch (index) {
        ...
        case 1:
          ...
          return <Step2Component patientDetails={patientDetails} handleInputChange={handleInputChange} />
        case 2:
          // console.log(`stepper 3: ${patientDetails.Fname}`)
          return <Step3Component patientDetails={patientDetails}/>
